...
sudo docker build --compress -t lisp-env -f lisp-env.dockerfile . 
sudo docker tag lisp-env docker.mercatusion.com:5000/lisp-env
sudo docker push docker.mercatusion.com:5000/lisp-env

sudo docker build --compress --no-cache --build-arg GITHUB_USER=$3 --build-arg GITHUB_PASS=$4 -t mercatus-core -f mercatus-core.dockerfile .
...
sudo docker build --compress --no-cache -t mercatus-client -f mercatus-client.dockerfile .
...
sudo docker build --compress --no-cache -t mercatus-service -f mercatus-service.dockerfile .
...
sudo docker build --compress --no-cache -t mercatus-repl -f mercatus-repl.dockerfile .
